prepare_list_table <- function(
  ...
  if (type == "Accuracy"){
    ...
    acc_frame_user %<>%
      ...
      dplyr::summarise(
        UserFirstInterval  = round(quantile(Accuracy, probs = 0.025), 4),
        UserMeanAccuracy   = round(mean(Accuracy), 4),
        UserMedianAccuracy = round(quantile(Accuracy, probs = 0.5), 4),
        UserSecondInterval = round(quantile(Accuracy, probs = 0.975), 4))
    acc_frame_prod %<>%
      dplyr::group_by(ClassId) %>%
      dplyr::summarise(
        ProdFirstInterval  = round(quantile(Accuracy, probs = 0.025), 4),
        ProdMeanAccuracy   = round(mean(Accuracy), 4),
        ProdMedianAccuracy = round(quantile(Accuracy, probs = 0.5), 4),
        ProdSecondInterval = round(quantile(Accuracy, probs = 0.975), 4))
